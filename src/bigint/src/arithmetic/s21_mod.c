...
static s21_decimal s21_integer_mod_private(s21_decimal dividend,
                                           s21_decimal divisor);
...
int s21_int_mod(s21_decimal dividend, s21_decimal divisor,
                s21_decimal *result) {
    if (eq_zero(divisor))
        return S21_NAN;
...
static s21_decimal s21_integer_mod_private(s21_decimal dividend,
    ...
    while ((s21_is_less_basic(divisor, dividend) ||
            s21_is_equal(divisor, dividend)) &&
           !get_bit(divisor, 95)) {
        // printf("1\n");
        shiftl(&divisor);
    }
